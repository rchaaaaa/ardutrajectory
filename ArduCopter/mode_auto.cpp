...
void ModeAuto::run()
{
    ...
    case Auto_TrajectoryTrack:
        trajectory_track_run();
        break;
    }
}
...
void ModeAuto::trajectory_track_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed())
    {
...
void ModeAuto::spline_run()
{
    ...
    Vector3f target_pos_traj, target_vel_traj, target_accel_traj;
    wp_nav->calc_timefunc_pos_vel_acc(target_pos_traj, target_vel_traj, target_accel_traj);

    Trajectory::reference reference(target_pos_traj, target_vel_traj, target_accel_traj);
