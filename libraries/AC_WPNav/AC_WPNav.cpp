...
const AP_Param::GroupInfo AC_WPNav::var_info[] = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("TYPE", 11, AC_WPNav, _wp_type, WPNAV_WP_TYPE),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("CT_OFSTX", 12, AC_WPNav, _wp_center_offset_x, 0.0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("CT_OFSTY", 13, AC_WPNav, _wp_center_offset_y, 0.0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("CT_OFSTZ", 14, AC_WPNav, _wp_center_offset_z, 0.0),
...
// their values.
//
AC_WPNav::AC_WPNav(const AP_InertialNav &inav, const AP_AHRS_View &ahrs, AC_PosControl &pos_control, const AC_AttitudeControl &attitude_control, AC_TrajTrack &traj_track) : _inav(inav),
                                                                                                                                                                             _ahrs(ahrs),
                                                                                                                                                                             _pos_control(pos_control),
                                                                                                                                                                             _traj_track(traj_track),
    ...
    if (!_flags.reached_destination)
    {
        if (_track_desired >= _track_length)
        {
            ...
            else
            {
                ...
                if (dist_to_dest.length() <= _wp_radius_cm && !_wp_type)
                {
                    _flags.reached_destination = true;
                }
                else if (dist_to_dest.length() <= 50 && _wp_type)
                {
                    _flags.reached_destination = true;
                }
            }
...
void AC_WPNav::update_timefunc_solution(const Vector3f &origin, const Vector3f &dest, const Vector3f &next_dest) //ned
{
    ...
    if (rotation)
        delete rotation;
    Vector3f vel_unit = (dest - origin).normalized();
    double distance = (dest - origin).length();
    double dist_vel, dist_accel;
    Vector3f vel_max, acc_max;
    vel_max.x = vel_unit.x > 0 ? _wp_speed_cms * 0.70710678 / 100 : -_wp_speed_cms * 0.70710678 / 100;
    vel_max.y = vel_unit.y > 0 ? _wp_speed_cms * 0.70710678 / 100 : -_wp_speed_cms * 0.70710678 / 100;
    vel_max.z = vel_unit.z > 0 ? _wp_speed_down_cms / 100 : -_wp_speed_up_cms / 100;
    acc_max.x = vel_unit.x > 0 ? _wp_accel_cmss * 0.70710678 / 100 : -_wp_accel_cmss * 0.70710678 / 100;
    acc_max.y = vel_unit.y > 0 ? _wp_accel_cmss * 0.70710678 / 100 : -_wp_accel_cmss * 0.70710678 / 100;
    acc_max.z = vel_unit.z > 0 ? _wp_accel_z_cmss / 100 : -_wp_accel_z_cmss / 100;
    vel_norm = vel_max * vel_unit;
    acc_norm = acc_max * vel_unit;
    ...
    {
        dist_vel = distance - dist_accel;
        t2 = t1 + dist_vel / vel_norm;
        t3 = t2 + t1;
        is_triangle = false;
    }
    else
    {
        t1 = sqrt(distance / acc_norm);
        t2 = t1;
        t3 = t2 + t1;
        is_triangle = true;
    }
    ...
    if ((next_dest - dest).length() > 0)
    {
        ...
        double angle = vel_unit.angle(next_vel_unit) * 180 / M_PI;
        double omega_max = MIN(sqrt(acc_norm / _wp_center_offset_max), vel_norm / _wp_center_offset_max);
        t_rot = angle * M_PI / 180 / omega_max;
        rotation = new Trajectory::RotationState(translation->getFinalPos(), dest);
        rotation->setRotate(dest, rot_axis, angle, Vector3f(0, 0, 0), 0, t_rot, t_rot, 0, true, 1, Trajectory::Color(), Trajectory::Color(), Trajectory::Color(), t3 + t0);
    }
}
...
void AC_WPNav::calc_timefunc_pos_vel_acc(Vector3f &pos, Vector3f &vel, Vector3f &accel)
{
    uint64_t now_usec_epoch;
    AP::rtc().get_utc_usec(now_usec_epoch);
    double now_sec_epoch = (now_usec_epoch)*0.000001;
    ...
    else if (elapsed_t > t0 + t3 && elapsed_t <= t_rot)
        rotation->getPos(elapsed_t, pos, vel, accel, color);
}
